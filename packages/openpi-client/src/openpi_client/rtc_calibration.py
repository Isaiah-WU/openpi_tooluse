...
class RTCDelayTracker:
    ...
    def __init__(self, initial_delay_policy_steps: int, *, maxlen: int = 20) -> None:
        if (
            ...
            raise ValueError(
                "initial_delay_policy_steps must be a non-negative integer"
            )
    ...
    def add(self, observed_delay_policy_steps: int) -> None:
        ...
        if (
            ...
            raise ValueError(
                "observed_delay_policy_steps must be a non-negative integer"
            )
...
def load_baseline_policy_delays(path: Path) -> list[float]:
    ...
    delays: list[float] = []
    path = Path(path)
    csv_paths = sorted(path.glob("**/request_timing.csv")) if path.is_dir() else [path]
    if not csv_paths:
    ...
    for csv_path in csv_paths:
        with csv_path.open(newline="", encoding="utf-8") as file:
            ...
            if (
                ...
                or "observed_delay_policy_steps" not in reader.fieldnames
            ):
                raise ValueError(f"{csv_path} is missing observed_delay_policy_steps")
...
def recommend_rtc_parameters(
    ...
    *,
    ...
    query_remaining = (
        int(math.ceil(float(p99))) + execution_horizon + policy_steps_per_control
    )
