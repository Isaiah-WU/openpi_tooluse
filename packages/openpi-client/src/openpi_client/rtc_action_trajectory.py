...
def resample_action_chunk(
    ...
    *,
    ...
    if actions.ndim != 2:
        raise ValueError("actions must be a 2D array, " f"got shape {actions.shape}")
    ...
    if len(actions) < 2 or abs(source_hz - target_hz) < 1e-6:
        return actions.copy()
    ...
    target_length = int(np.floor(end_time * target_hz + 1e-6)) + 1

    target_times = np.arange(target_length, dtype=np.float32) / float(target_hz)
...
def control_steps_to_policy_steps(
    ...
    *,
    ...
    policy_steps = control_steps * policy_hz / control_hz
...
def prepare_action_chunk(
    ...
    *,
    ...
    skipped_policy_steps = observed_delay_policy_steps if apply_rtc_delay_crop else 0
