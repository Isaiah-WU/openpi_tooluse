...
class RTCTrainingConfig:
    ...
    def __post_init__(self):
        if self.min_prefix_steps < 0:
            raise ValueError(
                "min_prefix_steps must be non-negative"
            )

        if (
            self.max_prefix_steps is not None
            and self.max_prefix_steps
            < self.min_prefix_steps
        ):
            raise ValueError(
                "max_prefix_steps must be greater than "
                "or equal to min_prefix_steps"
            )
        ...
        if (
            ...
            raise ValueError(
                "execution_horizon must be positive "
                "when specified"
            )

        if not 0.0 <= self.prefix_probability <= 1.0:
            raise ValueError(
                "prefix_probability must be in [0, 1]"
            )
...
class Pi0Config(_model.BaseModelConfig):
    ...
    rtc_training: RTCTrainingConfig = dataclasses.field(
        default_factory=RTCTrainingConfig,
    )
